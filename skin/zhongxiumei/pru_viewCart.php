...
<div class="viewCartFrame">
	...
	if (strlen($shizi) > 0 && strpos($shizi, "+")!=false)
		...
		$shizi = "";
	?>
		<li id="total" style="display:none">金额合计： <?php echo $shizi.$total.MONEY_DANWEI ;?></li>
		<li id="button" <?php if ($nCount<=1) echo "style=\"display:none;\"";?>><input class="submitBtn" type="button" value="提交订单" onclick="document.getElementById('button').style.display='none'; document.getElementById('submitForm').style.display='block';"/></li>
	</ul>
</div>
...
function change(id){
 ...
 <div class="TabbedPanelsContentGroup" id="submitForm" style="border-left:none; border-right:none;">
	  ...
	  <input name="url" type="hidden" value="?p=product&type=success" />
    </form>
</div>
